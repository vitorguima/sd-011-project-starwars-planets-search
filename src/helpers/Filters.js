...
export const sortColumns = (initialFilters, data) => {
  const { order } = initialFilters;
  const NEGATIVE_ONE = -1;
  switch (true) {
  case order.sort === 'ASC' && !isColumnTypeNumber(order.column):
    return data.sort((a, b) => ((a[order.column] > b[order.column]) ? 1 : NEGATIVE_ONE));
  case order.sort === 'DESC' && !isColumnTypeNumber(order.column):
    return data.sort((a, b) => ((a[order.column] < b[order.column]) ? 1 : NEGATIVE_ONE));
  case order.sort === 'ASC' && isColumnTypeNumber(order.column):
    return data.sort((a, b) => Number(a[order.column]) - Number(b[order.column]));
  case order.sort === 'DESC' && isColumnTypeNumber(order.column):
    return data.sort((a, b) => Number(b[order.column]) - Number(a[order.column]));
  default:
