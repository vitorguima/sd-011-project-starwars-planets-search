...
// Deixei o App funcionando como Provider
function App() {
    ...
    if (filters.filterByNumericValues.length > 0) { // Esse OKKKK
      arrayPlanets2 = results && results.filter((planet) => (
        filters.filterByNumericValues.some(({ column, comparison, value }) => (
          (comparison === 'menor que' && parseInt(planet[column], 10)
          < parseInt(value, 10))
          || (comparison === 'maior que' && parseInt(planet[column], 10)
          > parseInt(value, 10))
          || (comparison === 'igual a' && parseInt(planet[column], 10)
          === parseInt(value, 10))
        ))
      ));
